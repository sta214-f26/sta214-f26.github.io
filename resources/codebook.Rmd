...
Some of the R packages that we will use in this course include:

* The `tidyverse` packages (such as `ggplot2` and `dplyr`)
* `GGally` (for pairs plots)
* `statmod` (for quantile residuals)
* `regressinator` (for plot lineups)
* `topmodels` (for rootograms)
* `pscl` (for zero-inflated models)
* `lme4` (for mixed models)

(This list is not exhaustive). To install the `topmodels` package, you will need to run the following in the console:
...
To provide some useful helper functions for this course, I have also created a small `R` package for us to use, called `glmhelpR`. It can be installed in the console with:
...
(Uncomment the first line if you have not installed the `pak` package before).
...
titanic <- read_csv("https://sta214-f26.github.io/homework/Titanic.csv")
```
...
The fitted coefficients above can be interpreted on the *log odds* scale, or on the *odds* scale. **In this course, if asked to interpret logistic regression coefficients, you should interpret on the odds scale unless otherwise specified!**
...
**Odds:** For the odds, we exponentiate the estimated coefficients, and then interpret in terms of the percent change in the odds.
...
* Since $\exp\{-0.093\} = 0.911$, the odds change by a factor of 0.911, which is equivalent to an 8.9% decrease. So, we would say: "A one mm increase in flipper length is associated with an estimated decrease of 8.9% in the odds that a penguin is male, holding body mass constant."
* $\exp\{0.003\} = 1.003$, which is a 0.3% increase. So: "A one g increase in body mass is associated with an estimated increase of 0.3% in the odds that a penguin is male, holding flipper length constant."
...
We will use the `emp_logit_plot` function in the `glmhelpR` package to create empirical logit plots. Here is an example of using the function with the titanic data:

```{r}
library(glmhelpR)

emp_logit_plot(titanic, Survived ~ Fare, num_bins = 20)
```
...
```{r}
emp_logit_plot(titanic, Survived ~ sqrt(Fare), num_bins = 20)
```
...
It is hard to tell -- maybe the results look a bit better?

### Plot lineup

To better assess whether our model is a good fit, we can make a **lineup** of quantile residual plots. **One** of these plots corresponds to the original data, while all the **other** plots correspond to simulated data for which the model is correct. If we can identify *which* of the plots is from the original data, then our model is not correctly specified (and we need to consider modifications like transformations).

Here is a lineup with 20 residual plots for a logistic regression model of Titanic survival on Fare:

```{r, include=F}
set.seed(692345) # setting a seed for reproducibility
```
...
qresid_fn <- function(f){
  data.frame(resid = statmod::qresid(f),
             x = model.frame(f)$Fare)
}

model_lineup(m1, fn = qresid_fn, nsim = 20) |> 
  ggplot(aes(x = x, y = resid)) +
  geom_point(size = 0.5) +
  geom_smooth() +
  facet_wrap(vars(.sample)) +
  labs(x = "Fare", y = "Residual") +
  theme_bw()
```

To me, none of the plots particularly stand out as different than the others, and I'm not able to easily identify which plot corresponds to the original data. If I had to guess, I would choose plot 1, plot 5, or plot 14, but I wouldn't be confident in my guess. This suggests that the model assumptions might be reasonable.

If we want to check which of the plots corresponds to the original data, we can use the `decrypt` function from the `regressinator` package. Copy the `decrypt` message from the output, and run it in the console:

```{r}
decrypt("QUg2 qFyF Rx 8tLRyRtx KK")
```

Note that we will have to change the code if we fit a different model. For example, if we wanted to use sqrt(Fare) instead:
...
```{r, fig.width=8, fig.height=6}
# change the original model definition
m2 <- glm(Survived ~ sqrt(Fare), data = titanic, family = binomial)

# function to extract residuals needs to change
# The variable to plot on the x-axis is now sqrt(Fare)
# Since sqrt(Fare) includes special characters, need to put 
# the tick marks around the variable name
qresid_fn <- function(f){
  data.frame(resid = statmod::qresid(f),
             x = model.frame(f)$`sqrt(Fare)`)
}

# first argument is now m2, instead of m1
# Also need to change x-axis label (in labs)
model_lineup(m2, fn = qresid_fn, nsim = 20) |> 
  ggplot(aes(x = x, y = resid)) +
  geom_point(size = 0.5) +
  geom_smooth() +
  facet_wrap(vars(.sample)) +
  labs(x = "sqrt(Fare)", y = "Residual") +
  theme_bw()
```

As above, I can't tell which plot corresponds to the true data. It turns out it is plot 19:

```{r}
decrypt("QUg2 qFyF Rx 8tLRyRtx Zj")
```
...
sba <- read_csv("https://sta214-f26.github.io/class_activities/sba_small.csv")
...
```{r, include=F}
heart_data <- read_csv("https://sta214-f26.github.io/class_activities/framingham.csv")
