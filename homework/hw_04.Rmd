...
[https://sta214-f26.github.io/homework/hw_04_template.qmd](https://sta214-f26.github.io/homework/hw_04_template.qmd)
...
Using the output provided for the two models, estimate $SE(\widehat{\beta}_1)$ for the *second* model, `m2`.
:::

### Diagnostics

Your friend is interested in doing model diagnostics for a simple logistic regression model. They know that the log-odds are assumed to be a linear function of the explanatory variable. They also know that they can get the *fitted* log-odds for the model in R. 

To assess the shape assumption, they therefore decide to write the following code, which produces the plot below:
...
dengue |>
  mutate(fitted_logit = predict(m_dengue, type = "link")) |>
  ggplot(aes(x = WBC, y = fitted_logit)) +
  geom_point(size=0.5) +
  theme_bw() +
  labs(x = "WBC", y = "Estimated log-odds of dengue")
```

:::{.question}
#### Question 3

Examining the plot, they are ecstatic to see a perfectly linear relationship! They conclude that the shape assumption is satisfied.

Comment on your friend's approach.
:::

### Likelihood

The estimated coefficients for the model fit in question 3 are $\widehat{\beta}_0 = 1.737$ and $\widehat{\beta}_1 = -0.361$.

:::{.question}
#### Question 4

Suppose that instead of these estimated coefficients, I decide I prefer $\widehat{\beta}_0 = 1.5$ and $\widehat{\beta}_1 = -0.5$ (on the same data). If I calculate the log-likelihood for the model with these different coefficients (instead of the original coefficients 1.737 and -0.361), how will it change? (Explain whether it will increase, decrease, or stay the same).
:::
...
Suppose we have independent samples $(X_1, Y_1), ..., (X_n, Y_n)$ from the linear regression model

$$Y_i = \beta_0 + \beta_1 X_i + \varepsilon_i \hspace{1cm} \varepsilon_i \sim N(0, 1)$$

This is equivalent to saying that $Y_i \sim N(\beta_0 + \beta_1 X_i, 1)$, which means that

$$P(Y_i| \beta_0, \beta_1) = \frac{1}{\sqrt{2 \pi}} \exp\left\lbrace-\frac{1}{2} (Y_i - \beta_0 - \beta_1 X_i)^2 \right\rbrace$$

We now have two parameters to estimate ($\beta_0$ and $\beta_1$), but our likelihood is constructed the same way as before:

$$L(\beta_0, \beta_1) = \prod \limits_{i=1}^n P(Y_i| \beta_0, \beta_1)$$

:::{.question}
#### Question 8

Using this information, show that the log likelihood is

$$\log L(\beta_0, \beta_1) = n \log \left(\frac{1}{\sqrt{2 \pi}} \right) - \frac{1}{2} \sum \limits_{i=1}^n (Y_i - \beta_0 - \beta_1 X_i)^2$$

Then, argue that the log likelihood (or equivalently the likelihood) is maximized if we *minimize* the residual sum of squares:

$$SSE = \sum \limits_{i=1}^n (Y_i - \beta_0 - \beta_1 X_i)^2$$
:::
