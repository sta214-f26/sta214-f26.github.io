...
```{r, echo=F}
ct |>
  select(YEAR, Age, FEMALE, SurgeryType, BMI,
         CharlsonScore, CHF, VALVE, RENLFAIL, LIVER, METS, TUMOR, COAG, OBESE, LYTES, SurgDuration, Open, TWATemp, LastReadingTemp, EndCaseTemp, SeriousInfection,
...
p7 <- ct |>
  ggplot(aes(x = SurgDuration)) +
  geom_histogram(fill = "lightgrey", color = "black") +
  theme_classic()

p8 <- ct |>
  ggplot(aes(x = DurationHosp)) +
  geom_histogram(fill = "lightgrey", color = "black") +
  theme_classic()
